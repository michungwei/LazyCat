...
if(isset($_POST["oldpass"])){
	...
	$sql = "SELECT admin_password 
			FROM $table_admin 
			WHERE admin_account = '".$_SESSION["madmin"]."' AND admin_password = '$oldpass'";
	$row = $db -> query_first($sql);
	if($row){
		$db -> query("UPDATE $table_admin SET admin_password = '$newpass' WHERE admin_account = '".$_SESSION["madmin"]."'");
		script("密碼修改成功!!");
	}else{
		script("對不起,舊密碼錯誤!!");
	}
...
$(document).ready(function(){
	$("form").submit(function(){
		var re = true;
		err_msg = '';
		if(re){re = isnull("oldpass", "舊密碼", 0, 1, 30);}
		if(re){re = isnull("newpass", "新密碼", 0, 1, 30);}
		if(re){re = isnull("newpass2", "確認新密碼", 0, 1, 30);}
