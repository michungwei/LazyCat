...
if($row){
	$title = $row["lookbook_title"];
	$pic = $row["lookbook_titlepic"];
	$is_show = $row["lookbook_is_show"];
	$create_time = $row["lookbook_create_time"];
	$update_time = $row["lookbook_update_time"];
}else{
 	script("資料不存在");
}
...
$(document).ready(function(){
	$("form").submit(function(){
		var re = true;
		if(re){re = isnull("title", "名稱", 0, 1, 50);}
            ...
            <div class="listshow">
                <form action="edit_save.php?<?php echo $query_str; ?>" method="post" enctype="multipart/form-data" name="form" id="form">
                    ...
                    <table width="850" border="0" cellpadding="0" cellspacing="3">
                        ...
                        <tr>
                            <td width="150" valign="top"><h4 class="input-text-title">代表圖片</h4></td>
                            <td><p>
