...
$style_id = get("style_id", 1) != "" ? get("style_id", 1) : 1;
$row_pic = Banner::getPicFormId($style_id);
...
if($keyword != ""){
	...
	$rows_serial = Product::getStyleSerialList($style_id);
	if(count($rows_serial) <= 0){
		script("尚無商品!");
	}
	...
	$row_hprice = Product::getStyleMAXPrice($style_id, $serial_id);
	$row_lprice = Product::getStyleMINPrice($style_id, $serial_id);
	...
	$query_string = "?style_id=".$style_id."&serial_id=".$serial_id."&max_value_price=".$max_value_price."&min_value_price=".$min_value_price."&order=".$order;
	$sql = Product::getStyleProductList($style_id, $serial_id, $max_value_price, $min_value_price, $order);
...
<script src="assets/scripts/style_list.js"></script>
...
$(document).ready(function(e) {	
    select_price(max_price, min_price, max_value_price, min_value_price, "style");
	selectOrder(order);
...
<input name="style_id" type="hidden" id="style_id" value="<?php echo $style_id; ?>">
...
<div class="product-wrap">
    <div class="prod-type">
        ...
        <ul class="type-list">
			...
			?>
            <li class="type-item"><a href="style-list.php?keyword=<?php echo $keyword; ?>&max_value_price=<?php echo $row_serial["height_price"]; ?>&min_value_price=<?php echo $row_serial["low_price"]; ?>&order=9&page=1"><?php echo $row_serial["productserial_name"]; ?> <span class="type-num">(<?php echo $row_serial["amount"]; ?>)</span></a></li>
            <?php
				}
			?>
        </ul>
		...
		?>
        <h3 class="prod-title"><?php echo $row_pic["pic_name_tw"].' / '.$row_pic["pic_name_en"]; ?></h3>
        <ul class="type-list">
            <?php
                $amount_sum = 0;
				foreach($rows_serial as $row_serial){
			?>
            <li class="type-item"><a href="style-list.php?style_id=<?php echo $style_id; ?>&serial_id=<?php echo $row_serial["productserial_id"]; ?>&max_value_price=<?php echo $row_serial["height_price"]; ?>&min_value_price=<?php echo $row_serial["low_price"]; ?>&order=9&page=1"><?php echo $row_serial["productserial_name"]; ?> <span class="type-num">(<?php echo $row_serial["amount"]; ?>)</span></a></li>
            <?php
                    $amount_sum += $row_serial["amount"];
				}
			?>
            <li class="type-item"><a href="style-list.php?style_id=<?php echo $style_id; ?>">All <span class="type-num">(<?php echo $amount_sum; ?>)</span></a></li>
        </ul>
        <?php
		}
		?>
